...
export class VetMenuFormComponent extends BaseFormService<VetMenuFormModel, VetMenuModel> {
  ...
  override onSubmit(model: VetMenuModel) {
    if (model.vetSettings) {
      ...
      this.store.dispatch(
        setRoute({ page: RoutePageEnum.dashboardVetServices, section: RouteSectionEnum.dashboardVetServices })
      );
      ...
      this.store.dispatch(
        setRoute({ page: RoutePageEnum.dashboardVetPatients, section: RouteSectionEnum.dashboardVetPatients })
      );
    }
  }
}
